...
```
### [String manipulation functions](http://www.gnu.org/software/gawk/manual/html_node/String-Functions.html)
```awk
asort(); length(); gsub; split()....
```
### Output
```awk
...
system("date") # execute "date" command. Note double quotes around the Unix command
```

If you have reached this point, perhaps you are already asking where you can read something to begin with.
This site is perhaps good for beginners: https://en.wikibooks.org/wiki/AWK
